...
mongostr = "mongodb://mongo.loc:27017"
db_name = "blog"
...
def create_user(username, password):
    ...
    try:
        ...
        request = urllib.request.Request(url=url, data=data.encode("utf-8"))
...
def add_blog_post(title, post, tags):
    try:
        ...
        data = urllib.parse.urlencode([("body", post), ("subject", title), ("tags", tags)])
...
def add_blog_comment(title, post):
    try:
        ...
        data = urllib.parse.urlencode([("commentName", comment_name), ("commentBody", comment_body), ("permalink", permalink)])
        request = urllib.request.Request(url=url, data=data.encode("utf-8"))
        cj.add_cookie_header(request)
...
# Trying to create a test user  lMDasIh
# Found the test user  lMDasIh  in the users collection
# User creation successful.
# Trying to login for test user  lMDasIh
# User login successful.
# Trying to submit a post with title  JQQwqPPAyMfZkmGunCziHfuBecoFKh
# Submission of single post successful
# Trying to submit a post with title  fdzwqyYFnoLCKppwmMksWciylJKaMb
...
# Trying to submit a blog comment for post with title JQQwqPPAyMfZkmGunCziHfuBecoFKh
