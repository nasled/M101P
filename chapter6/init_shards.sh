...
mongod --logpath "cfg-a.log" --dbpath /data/config/config-a --port 60017 --fork --configsvr --smallfiles --replSet cfg
mongod --logpath "cfg-b.log" --dbpath /data/config/config-b --port 60018 --fork --configsvr --smallfiles --replSet cfg
mongod --logpath "cfg-c.log" --dbpath /data/config/config-c --port 60019 --fork --configsvr --smallfiles --replSet cfg

sleep 5

echo "Configuring cfg replica set"
mongo --port 60017 << 'EOF'
config = { _id: "cfg", members:[
          { _id : 0, host : "localhost:60017" },
          { _id : 1, host : "localhost:60018" },
          { _id : 2, host : "localhost:60019" }]};
rs.initiate(config)
EOF

mongos --logpath "mongos-1.log" --configdb cfg/localhost:60017,localhost:60018,localhost:60019 --port 12345 --fork
echo "Waiting 60 seconds for the replica sets to fully come online"
sleep 60

echo "Connnecting to mongos and enabling sharding"
## add shards and enable sharding on the test db
mongo --port 12345 <<'EOF'
sh.addShard("s0/" + "localhost:37017");
sh.addShard("s1/" + "localhost:47017");
sh.addShard("s2/" + "localhost:57017");
sh.enableSharding("school");
sh.shardCollection("school.students", {student_id:1});
EOF